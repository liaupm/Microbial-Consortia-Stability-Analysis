...
def ann(cleanattributes, onehot, df, n_epochs, learning_rate, batch_size, n_neurons_per_hlayer, dropout, act_fun, my_regularizer, my_initializer, my_optimizer, norm, n):
    ...
    model.add(keras.layers.Dense(output_size_neuralnetwork, activation="softmax"))
    ...
    for repeat in range(5):
        ...
        acc_train = results.categorical_accuracy.values[-1:][0]
        acc_val =  results.val_categorical_accuracy.values[-1:][0]
...
if __name__ == '__main__':
    ...
    cleanattributes = 'C:/TFM/ann/data_replating_clean.csv'
    onehot = 'C:/TFM/ann/data_replating_onehot_2clases.csv'
    ...
    lr= [0.0001, 0.001]
    batch = [512, 1024] #, 5018]
    n_neurons_per_hlayer = [75, 50, 25]
    dr = [0.25, 0.1]
    act = ['relu', 'elu', 'tanh'] #tanh
